...
/* ---------------------------------------------------------
   Product imagery — generated locally as inline SVG data URIs.
   No network request, no external image host, so the catalog
   always renders instantly with no broken images or 403s.
   Swap these for real photo URLs any time (see README).
   --------------------------------------------------------- */
const CATEGORY_THEME = {
  clothing: { from:'#1F2B25', to:'#0E1512', accent:'#C8A24A' },
  wine:     { from:'#3A1420', to:'#12080B', accent:'#D9B36C' },
  beads:    { from:'#2B2013', to:'#120E08', accent:'#E7C77A' }
};

const CATEGORY_ICON_PATHS = {
  // simple line-art icons, drawn in a 0..200 box, centered later
  clothing: 'M70 20 L100 40 L130 20 L150 45 L130 60 L130 180 L70 180 L70 60 L50 45 Z M85 40 Q100 55 115 40',
  wine: 'M85 15 H115 V45 Q115 65 130 80 Q140 95 140 120 V175 Q140 185 130 185 H70 Q60 185 60 175 V120 Q60 95 70 80 Q85 65 85 45 Z M85 15 V45 M115 15 V45 M65 130 H135',
  beads: 'M100 25 C75 25 60 45 60 65 C60 85 75 90 75 105 C75 120 60 125 60 145 C60 165 75 185 100 185 C125 185 140 165 140 145 C140 125 125 120 125 105 C125 90 140 85 140 65 C140 45 125 25 100 25 Z'
};

function svgProductImage(product, variantLabel){
  const theme = CATEGORY_THEME[product.category] || CATEGORY_THEME.clothing;
  const icon = CATEGORY_ICON_PATHS[product.category] || CATEGORY_ICON_PATHS.clothing;
  const title = escapeHTML(product.name);
  const label = escapeHTML(variantLabel || product.category);
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 750">
    ...
    <rect x="24" y="24" width="552" height="702" fill="none" stroke="${theme.accent}" stroke-opacity="0.35" stroke-width="1.5"/>
    <g transform="translate(200,150) scale(1)" fill="none" stroke="${theme.accent}" stroke-width="3" stroke-linejoin="round" stroke-linecap="round">
      <path d="${icon}"/>
    </g>
    <text x="300" y="560" text-anchor="middle" font-family="Georgia, serif" font-size="30" font-weight="600" fill="${theme.accent}">${title}</text>
    <text x="300" y="596" text-anchor="middle" font-family="Georgia, serif" font-size="17" letter-spacing="2" fill="#F4EFE3" fill-opacity="0.65">${label.toUpperCase()}</text>
  </svg>`;
  return 'data:image/svg+xml;utf8,' + encodeURIComponent(svg);
}

function productGalleryImages(product){
  const labels = ['Front', 'Detail', 'Styled'];
  return labels.map(label => svgProductImage(product, label));
}

// Attach generated imagery to every product now that the generator above is defined.
PRODUCTS.forEach(p => { p.images = productGalleryImages(p); });
